...
public class Mojing1stController : MonoBehaviour
{
    ...
    private Vector2 GetInput()
    {
        float x = 0.0f;
        float y = 0.0f;
        if (MojingInputManager.Instance.isTouchDown)
        {
            x = CrossPlatformInputManager.GetAxis("Horizontal") * 2.0f - 1;
            y = -(CrossPlatformInputManager.GetAxis("Vertical") * 2.0f - 1);
        }
        else
        {
    ...
    void Update()
    {
        ...
        mainTransform.position = this.transform.position + new Vector3(0, camHeight, 0);
        //Debug.Log(mainTransform.position);
		...
		inputJump = CrossPlatformInputManager.GetButton("OK")|| CrossPlatformInputManager.GetButton("A");
#endif
        if (!useFixedUpdate)
            UpdateFunction();
    }
